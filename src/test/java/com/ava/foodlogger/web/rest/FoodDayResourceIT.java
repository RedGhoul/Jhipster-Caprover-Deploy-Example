...
class FoodDayResourceIT {
    ...
    void createFoodDay() throws Exception {
        ...
        // Create the FoodDay
        FoodDayDTO foodDayDTO = foodDayMapper.toDto(foodDay);
        restFoodDayMockMvc
            .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(foodDayDTO)))
    ...
    void putNewFoodDay() throws Exception {
        ...
        FoodDayDTO foodDayDTO = foodDayMapper.toDto(updatedFoodDay);

        restFoodDayMockMvc
            .perform(
                put(ENTITY_API_URL_ID, foodDayDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(TestUtil.convertObjectToJsonBytes(foodDayDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamFoodDay() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restFoodDayMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(foodDayDTO)))
    ...
    void patchNonExistingFoodDay() throws Exception {
        ...
        restFoodDayMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, foodDayDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(TestUtil.convertObjectToJsonBytes(foodDayDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamFoodDay() throws Exception {
        ...
        restFoodDayMockMvc
            .perform(
                patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(TestUtil.convertObjectToJsonBytes(foodDayDTO))
            )
            .andExpect(status().isMethodNotAllowed());
