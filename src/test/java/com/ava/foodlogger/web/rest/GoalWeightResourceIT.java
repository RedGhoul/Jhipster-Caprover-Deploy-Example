...
class GoalWeightResourceIT {
    ...
    void createGoalWeight() throws Exception {
        ...
        GoalWeightDTO goalWeightDTO = goalWeightMapper.toDto(goalWeight);
        restGoalWeightMockMvc
            .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(goalWeightDTO)))
    ...
    void putNewGoalWeight() throws Exception {
        ...
        GoalWeightDTO goalWeightDTO = goalWeightMapper.toDto(updatedGoalWeight);

        restGoalWeightMockMvc
            .perform(
                put(ENTITY_API_URL_ID, goalWeightDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(TestUtil.convertObjectToJsonBytes(goalWeightDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamGoalWeight() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restGoalWeightMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(goalWeightDTO)))
    ...
    void patchNonExistingGoalWeight() throws Exception {
        ...
        restGoalWeightMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, goalWeightDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(TestUtil.convertObjectToJsonBytes(goalWeightDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamGoalWeight() throws Exception {
        ...
        restGoalWeightMockMvc
            .perform(
                patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(TestUtil.convertObjectToJsonBytes(goalWeightDTO))
            )
            .andExpect(status().isMethodNotAllowed());
