...
class CurrentWeightResourceIT {
    ...
    void createCurrentWeight() throws Exception {
        ...
        CurrentWeightDTO currentWeightDTO = currentWeightMapper.toDto(currentWeight);
        restCurrentWeightMockMvc
            .perform(
                post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(currentWeightDTO))
            )
    ...
    void putNewCurrentWeight() throws Exception {
        ...
        CurrentWeightDTO currentWeightDTO = currentWeightMapper.toDto(updatedCurrentWeight);

        restCurrentWeightMockMvc
            .perform(
                put(ENTITY_API_URL_ID, currentWeightDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(TestUtil.convertObjectToJsonBytes(currentWeightDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamCurrentWeight() throws Exception {
        ...
        restCurrentWeightMockMvc
            .perform(
                put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(currentWeightDTO))
            )
    ...
    void partialUpdateCurrentWeightWithPatch() throws Exception {
        ...
        partialUpdatedCurrentWeight.weight(UPDATED_WEIGHT);
    ...
    void patchNonExistingCurrentWeight() throws Exception {
        ...
        restCurrentWeightMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, currentWeightDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(TestUtil.convertObjectToJsonBytes(currentWeightDTO))
            )
            .andExpect(status().isBadRequest());
